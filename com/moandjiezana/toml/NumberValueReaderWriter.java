   ...
   public Object read(String s, AtomicInteger index, Context context) {
      ...
      for(int i = index.get(); i < s.length(); i = index.incrementAndGet()) {
         ...
         if (Character.isDigit(c)) {
            ...
            if (c != '_' || !underscorable || !notLastChar || !Character.isDigit(s.charAt(i + 1))) {
               if (!terminatable) {
                  type = "";
               }
      ...
      if (type.equals("integer")) {
         ...
         return Double.parseDouble(exponentString[0]) * Math.pow(10.0D, Double.parseDouble(exponentString[1]));
      } else {

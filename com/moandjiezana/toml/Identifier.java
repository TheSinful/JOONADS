...
class Identifier {
   static final Identifier INVALID = new Identifier("", (Identifier.Type)null);
      ...
      if (name.trim().isEmpty()) {
         ...
         for(int i = 0; i < name.length(); ++i) {
            ...
            if (c != '"' || i != 0 && name.charAt(i - 1) == '\\') {
               if (!quoted && "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890_-".indexOf(c) == -1) {
                  context.errors.invalidKey(name, context.line.get());
                  return false;
               }
            } else {
